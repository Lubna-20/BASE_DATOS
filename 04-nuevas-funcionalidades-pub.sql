...
alter table EMPLEADO add column tipo_contrato varchar(25) constraint CHK_TIPO_CONTRATO check (tipo_contrato2 in('FIJO DISCONTINUO', 'INDEFINIDO', 'PRACTICAS'));
...
alter table EMPLEADO add constraint CHK_EDAD check (edad between 19 and 65); -- OJO TAMBIEN ASI --> check(edad > 18 and edad < 66)
-- Modifica la restriccion anterior para que sea mayor igual a 18. ¿Puedes?  -- LAS CONSTRAIN NO SE PUEDEN MODIFICAR TENDREMOS QUE BORRARLAS Y CREARLAS DE NUEVO 
...
rename table PUB to nombre_pub;
...
drop view INFO_PUB;
-- Borra las tablas que son copiasempleado
